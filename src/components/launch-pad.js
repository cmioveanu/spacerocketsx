...
export default function LaunchPad({ state, dispatch }) {
  ...
  return (
    <div>
      ...
      />
      <Header launchPad={launchPad} state={state} dispatch={dispatch} />
      <Box m={[3, 6]}>
        ...
        <RecentLaunches launches={launches?.docs} state={state} dispatch={dispatch} />
      </Box>
    </div>
  );
}
...
function Header({ launchPad, state, dispatch }) {
  return (
    <Flex
...
function RecentLaunches({ launches, state, dispatch }) {
  if (!launches?.length) {
    return null;
  }
  return (
    <Stack my="8" spacing="3">
      ...
      <SimpleGrid minChildWidth="350px" spacing="4">
        {launches.map((launch) => (
          <LaunchItem launch={launch} key={launch.flight_number} state={state} dispatch={dispatch} />
        ))}
      </SimpleGrid>
    </Stack>
  );
